...
int md_decode_meter_pack(uint_8 metmod, uint_8 salveaddr, uint_8 funcode, uint_8 len, uint_8 *buffer, char type)
{
    ...
    if ((int)inv_meter.invdata.con_stu < 0 || (int)inv_meter.invdata.e_today < 0)
    {
        inv_meter.invdata.con_stu = 0;
        inv_meter.invdata.e_today = 0;
    ...
    if (get_second_sys_time() > 180 && g_meter_data_sync_enable == 1)
    {
        g_meter_data_sync_enable = 0;
...
int query_meter_proc(int is_for_cloud)
{
    int ret = -1;
    ...
    if (mtconfig.enb == 1)
    {
        ...
        if (0x100 == (mtconfig.reg & 0XF00)) //regulate enable
        {
