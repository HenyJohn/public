...
int8_t protocol_ate_paraset_handler(char *body)
{
    ...
    /*
     [  add] if no value to set ap.ssid&ap.password
     to default value AISWEI-LANSTICK 12345678
     // 20220621
   */
    uint8_t mPsnLen = strlen(setting.psn);
    if (mPsnLen > 0 && mPsnLen < (sizeof(ap_para.ssid) - 1))
    {
        // kaco-lanstick 20220808 +
        // sprintf((char *)ap_para.ssid, "%s%s", "AISWEI-", setting.psn + strlen(setting.psn) - 4);
        memcpy(ap_para.ssid, setting.psn, strlen(setting.psn)+1);
        ap_para.ssid[strlen(setting.psn)] = '\0';
    }
    else
    {
...
int8_t protocol_ate_update_handler(char *body)
{
    ...
    if (strstr(url, "http") == NULL) //[  mark] 这里添加检测是？？？
        return -1;
    update_firmware(url);
