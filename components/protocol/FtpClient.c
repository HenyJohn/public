...
/* asw-kaco */
static int asw_xfer(const char  *buf_csv, const char *path,
					NetBuf_t *nControl, int typ, int mode, int len);
					...
					NetBuf_t *nControl, int typ, int mode, int len)
{
	...
	if (typ == FTP_CLIENT_FILE_WRITE)
	{
		memcpy(dbuf, buf_csv, len);
		int c = writeFtpClient(dbuf, len, nData);
		if (c < len)
		{
...
///-------------------- kaco-lanstick --------------- ///
/* return 1 if successful, 0 otherwise */
static int asw_putDataFtpClient(const char  *inputbuf, const char *path, char mode,
								NetBuf_t *nControl, int len)
{
	return asw_xfer(inputbuf, path, nControl, FTP_CLIENT_FILE_WRITE, mode, len);
}
...
FtpClient *getFtpClient(void)
{
	if (!isInitilized)
	{
		...
		ftpClient_.asw_ftpClientPut = asw_putDataFtpClient; // asw-kaco
