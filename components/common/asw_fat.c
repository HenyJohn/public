#include "asw_fat.h"
#include "diskio_wl.h" //   for ff_diskio_register_wl_partition
#include "kaco.h"
...
static void get_file_list_from_dir(char *path, cJSON *arr)
{
    ...
    while ((next_file = readdir(theFolder)) != NULL)
    {
        ...
        if (check_file_exist(filepath) < 0) //  [mark] define in the cloud come in soon
            continue;
                                                  ...
                                                  wl_handle_t *wl_handle)
{
    ...
    BYTE pdrv = 0xFF;
    if (ff_diskio_get_drive(&pdrv) != ESP_OK) //[mark]ff_diskio 跟fats有關的 未找到具体的使用说明文档
    {
    ...
    result = ff_diskio_register_wl_partition(pdrv, *wl_handle); //[mark]找不到此函数的定义 在 component/fats/vfs/文件下 头文件 [https://github.com/jkearins/ESP32_mkfatfs]
    if (result != ESP_OK)
    {
...
int write_lost_data(Inv_data *inv_data, Inv_data *meter_data)
{
    ...
    // kaco-lanstick 20220802 +
    int len = fwrite(buff, sizeof(char), sizeof(Inv_data) + 2, fp);

    if (len > 200 && meter_data->status == 0 && strlen(meter_data->psn))
    {
        memset(buff, 0, sizeof(buff));
        memcpy(buff, (char *)meter_data, sizeof(Inv_data));
...
////////////////////////////////////////////////////////
/**
 * @brief asw_kaco_write_lost_data 写离线数据
 *         kaco-lanstick v1.0.1
 * @param invData
 * @param meterData
 * @return int
 */
extern char DEMO_DEVICE_NAME[IOTX_DEVICE_NAME_LEN + 1];
#define DEMO_DEVICE_NAME_TEST "B3278A260749" /// JST FOR TEST
//////////////////////////////////////////////////////////////////

int asw_kaco_add_datato_file(bool bIsFoundMeter, kaco_inv_data_t *pkcInvData, Inv_data *meterData, char *fileName)
{

    if (check_inv_pv(&pkcInvData->crt_inv_data) < 0)
        return -1;
    char mpFileDataBuf[1024] = {0}; // mark size if OK ??? TO TEST

    /* 离线数据保存流程 逆变器&电表两个文件进行存储*/
    ...
    {
        if (strcmp(pkcInvData->crt_inv_data.psn, kaco1_dev_psn[i]) == 0)
        {
            memset(pkcInvData->crt_inv_data.psn, 0, sizeof(pkcInvData->crt_inv_data.psn));
            memcpy(pkcInvData->crt_inv_data.psn, kaco1_conv_psn[i], strlen(kaco1_conv_psn[i]) + 2);
            // printf("\n============963 kaco test inv psn %s conver to :%s\n", kaco1_dev_psn[i], pkcInvData->crt_inv_data.psn);
        }
    }
    //////////////////////////////////////////////////////////////////
    ...
    //-1- 判断对应的逆变器序列号的文件是否存在
    if (access(fileName, 0) != 0)
    {
        ESP_LOGE("-- Open CSV File ERRO --", "cannot found the file %s ", fileName);

        return 0;
    }

    FILE *fp = fopen(fileName, "a+"); //"wb"
    if (fp == NULL)
    {
        ESP_LOGE("-- Create CSV File ERRO --", "Failed to open %s file with ab+ ", fileName);
        return -1;
    }

    //-3- 判断文件大小--过大 覆盖
    /* 获取前面一半的数据，删除 */
    if (ftell(fp) > 800000) /* TODO */
    {
    }

    //-4- 追加数据到对应的文件中;

    int mpFbufLen = asw_kaco_format_csv_data(mpFileDataBuf, bIsFoundMeter, &pkcInvData->crt_inv_data, meterData, pkcInvData->crt_inv_index);
    if (mpFbufLen <= 0) // mark size if OK ??? TO TEST
    {
        ESP_LOGE("-- Create Kaco Data ERRO --", "  %s data size is less than 0\n", fileName);
        fclose(fp);
        return -1;
    }

    int mLenWrtDataFile = fprintf(fp, mpFileDataBuf);
    if (mpFbufLen != mLenWrtDataFile)
    {

        ESP_LOGE("-- Write  CSV File ERRO --", "failed write data size %d/%d to  %s file", mpFbufLen, mLenWrtDataFile, fileName);
        fclose(fp);
        return -1;
    }
...
extern char metermodesn[7][10];
////////////////////////////////////////////////////////////////
int asw_kaco_write_lost_data(kaco_inv_data_t *pkcInvData, Inv_data *meterData)
{
    ...
    for (uint8_t i = 0; i < 5; i++)
    {
        if (strcmp(pkcInvData->crt_inv_data.psn, kaco1_dev_psn[i]) == 0)
        {
            ...
            memcpy(pkcInvData->crt_inv_data.psn, kaco1_conv_psn[i], strlen(kaco1_conv_psn[i]) + 2);
            // printf("\n============ 852 kaco test inv psn %s conver to :%s\n", kaco1_dev_psn[i], pkcInvData->crt_inv_data.psn);
        }
    }
    //////////////////////////////////////////////////////////////////
    ...
    if (bIsFoundMeter)
    {
        sprintf(fileName, "/inv/%s_meter1.csv", pkcInvData->crt_inv_data.psn);
        ...
        {
            ESP_LOGE("--Kaco Meter ERRO --", "Meter mode index is out range [0,5] %d", meter_mode_index);
            return -1;
        }
    }
    else
    {
        sprintf(fileName, "/inv/%s_meter0.csv", pkcInvData->crt_inv_data.psn);
    }

    ESP_LOGI("--- KACO History ---", " write_lost_data filename:%s", fileName);

    //-1- 判断对应的逆变器序列号的文件是否存在
    if (access(fileName, 0) != 0)
    {
        //-2- 不存在，新建并添加csv文件表头内容;
        /* TODO */

        ESP_LOGW("--Kaco file Manage --", "%s is note exited, will be created ", fileName);
        char mpFileBuf[3072] = {0}; // mark size if OK ??? TO TEST
        static Stu_KacoCsvHeader mStu_KacoCSVHeader;

        memcpy(mStu_KacoCSVHeader.dataloger, DEMO_DEVICE_NAME_TEST, sizeof(DEMO_DEVICE_NAME_TEST));
        memcpy(mStu_KacoCSVHeader.invsn, pkcInvData->crt_inv_data.psn, sizeof(pkcInvData->crt_inv_data.psn));
        mStu_KacoCSVHeader.crt_index = pkcInvData->crt_inv_index;

        if (bIsFoundMeter)
        {
            assert(pc_meter_mode_sn != NULL);
            memcpy(mStu_KacoCSVHeader.metersn, pc_meter_mode_sn, strlen(pc_meter_mode_sn) + 1);
        }

        int mDataLen = asw_kaco_format_csv_header(mpFileBuf, bIsFoundMeter, &mStu_KacoCSVHeader);
        if (mDataLen < 500) // mark size if OK ??? TO TEST
        {
            ESP_LOGE("-- Create Kaco Data ERRO --", "  %s header size is less than 500\n", fileName);
            return -1;
        }
        ...
        if (fp == NULL)
        {
            ESP_LOGE("-- Create  CSV File ERRO --", "Failed to open %s file with w ", fileName);
            return -1;
        }
        assert(strlen(mpFileBuf) > 0);
        int mLenWrtFile = fprintf(fp, mpFileBuf);
        if (mLenWrtFile != mDataLen)
        {
            ESP_LOGE("-- Write  CSV File ERRO --", "failed write size %d/%d to  %s file", mDataLen, mLenWrtFile, fileName);
            fclose(fp);
            return -1;
        }
        fclose(fp);
...
#if 0
        /* 写入历史文件名到文件列表中 */
        int res = asw_add_fileName_to_historyfiles(fileName);
        if (res == -1)
        {
            ESP_LOGE("-- Kaco Add History File ERRO -- ", "failed add history filename to fileslist.");
            g_num_failed_write_csv++; // Mark TODO 数据丢失错误 如何处理？？
        }
        usleep(10 * 1000);     // 10ms need or not ??
        asw_printf_filelist(); // FOR TEST DEBUG PRINT
#endif
    }
    ...
    // fseek(fp, 0, SEEK_END);
    ESP_LOGI("-- Kaco History File Info --"," %s File size:%d", fileName,size);
...
EXT_RAM_ATTR char exram_buf[5900] = {0}; //[  mem]add
//--------------------------------
