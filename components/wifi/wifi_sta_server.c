...
#define CONFIG_APSTA_STA_SSID "11111111"
#define CONFIG_APSTA_STA_PASSWORD "2222222222"
...
int get_eth_connect_status(void)
{
    ...
    if (s_net_event_group == NULL)
        return -1;
    var = xEventGroupGetBits(s_net_event_group);
    if (var & ETH_CONNECTED_BIT) // [lan]add ---ETH_CONNECTED_BIT
    {
        return 0;
    }
    else
...
static void display_scan_result(void)
{
    ...
    uint16_t ap_count = 0;
    int index = 0;
    char *pCssid; //   add for ssid unit8 to char
    cJSON *ap_arr = NULL;
    cJSON *item = NULL;
...
int find_now_apname(void)
{
    ...
    if (strstr(p, sta_save_ssid) != NULL)
    {
        printf("\n found router %s \n", sta_save_ssid);
        return 0;
    }
...
{
    memcpy(buff, hostcomm.ssid, strlen((char *)hostcomm.ssid) + 1); // [  mark]
}
...
static void asw_wifi_ap_mode()
{
    ...
    wifi_config_t wifi_ap_config = {
        .ap = {
            ...
            .ssid_len = 32,           // strlen(CONFIG_APSTA_AP_SSID),
...
static void asw_wifi_sta_mode()
{
    ...
    // s_asw_esp_netif = esp_netif_create_wifi(WIFI_IF_STA, &esp_netif_config);
...
void asw_wifi_apscan_mode()
{
    ...
    ESP_LOGE(TAG, "  ap scan job:  ap_config,ssid:%s,pawd:%s,channel:%d",
             (char *)wifi_ap_config.ap.ssid, (char *)wifi_ap_config.ap.password, wifi_ap_config.ap.channel);
...
void asw_net_start(void)
{
    ...
    if (g_state_ethernet_connect < 1)
    {
        ...
        printf("\n =====  Lan-ETH is not checked !!! work_mode:%d===== \n", g_stick_run_mode);
...
void net_http_server_start(void *pvParameters)
{
    ...
    start_kaco_tcp_server(); // kaco-lantick 20220801 +
