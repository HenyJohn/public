...
extern int trans_resp_pub(cloud_inv_msg *resp, unsigned char *ws);
...
int query_data_proc()
{
    ...
    if (RST_OK == res)
    {
        ...
        if (strlen(inv_arr[inv_index - 1].regInfo.msw_ver) < 1)
...
int scan_register(void)
{
    ...
    while (1)
    {
        ...
        if (abort_cnt > 22 || 1 == scan_stop) //|| ((esp_timer_get_time() - scan_start_time_sec) > 300))// || 1 == scan_stop) //5min timeout
        {
...
int get_mach_type_by_psn(char *inv_sn)
{
    for (int j = 0; j < INV_NUM; j++)
    {
        if (strstr(inv_arr[j].regInfo.sn, inv_sn) != NULL)
        {
            return inv_arr[j].regInfo.mach_type;
        }
    }
