...
void trim_space(char *str, int size)
{
    char *head = NULL;
    char *tail = NULL;
    char *new_str = NULL;
...
void md_decode_inv_data_content(uint8_t *packet, Inv_data *inv_data_ptr)
{
    ...
    inv_data_ptr->error = ((uint8_t)packet[154] << 8) | (uint8_t)packet[155];                                                           ///< Failure description for status 'failure'
#ifdef PRT_DATA
...
void Check_upload_unit32(const char *field, uint_32 value, char *body, uint_16 *datalen)
{
    uint_16 len = (*datalen);
    if (value != 0xFFFFFFFF)
    {
