...
int md_query_inv_data(Inverter *inv_ptr, unsigned int *time, unsigned short *error_code)
{
    ...
    inv_data.rtc_time = get_time(now_time, sizeof(now_time)) - 8 * 3600; //beijing to unix
    ...
    if (strlen(inv_data.psn) > 10 && get_second_sys_time() > 180)
        save_inv_data(inv_data, time, error_code);

    return res;
}

int get_current_inv_total_power(void)
{
    int ptw = 0;
    for (int k = 0; k < inv_real_num; k++)
        ptw += cgi_inv_arr[k].invdata.pac;
    return ptw;
}
...
int save_inv_data(Inv_data p_data, unsigned int *time, unsigned short *error_code)
{
    ...
    if ((((_data.time[0] - 0x30) * 10 + (_data.time[1] - 0x30)) * 100 + (_data.time[2] - 0x30) * 10 + (_data.time[3] - 0x30)) < 2020)
        return 0;
...
sent_error:
    ...
    //if (0 == netework_state)
    {
        ...
        if (mq0 != NULL)
        {
            if (is_in_cloud_main_loop == 1 && cat1_get_mqtt_status(0) == 1 && cat1_get_data_call_status() == 1)
            {
                g_meter_data_sync_enable = 1;
                xQueueSend(mq0, (void *)&_data, (TickType_t)0);
                printf("mqueue send ok\n");
            }
            else
            {
                if (xSemaphoreTake(invdata_store_sema, (TickType_t)10000 / portTICK_RATE_MS) == pdTRUE)
                {
