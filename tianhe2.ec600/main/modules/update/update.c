...
void update_monitor(char *url)
{
    int res = -1;
    int file_len = 0;
    res = http_get_file(url, "/home/update.bin", &file_len);
    if (res == 0)
    {
        res = sha256_file_check("/home/update.bin");
        if (res == 0)
        {
            update_pmu();
            esp_restart();
...
void download_task(void *pvParameters)
{
    ...
    //char str[256]={0};
    //memcpy(str, mqtt_url->down_url, strlen(mqtt_url->down_url));
    //printf("update process get download_url--------------------->%s-%d--%s-----%d--- \n", strstr(mqtt_url.down_url, "http://"),strlen(mqtt_url.down_url), mqtt_url.down_url, mqtt_url.update_type);
    ...
    if (strlen(mqtt_url.down_url) > 10)
    {
        ...
        {
            update_monitor(mqtt_url.down_url);
        }
        ...
        if (mqtt_url.update_type == 2)
        {
            ...
            res = http_get_file(mqtt_url.down_url, "/home/inv.bin", &file_len);
            if (res != 0)
                esp_restart();
            ...
            if (fp != NULL)
            {
                ...
                //     fread(buf, 1, 50, fp); //每次读取50字节
                //     //printf("%s\n", buf);
                // }
                fclose(fp);
                printf("httphead len %d  downloadfile len %d \n", file_len, lSize);
...
int get_update_file_name(char *url, char *file_name)
{
    ...
    //memcpy(file_name, savename, strlen(savename));
    return 0;
}

char p2p_upgrade_inv_enable = 0;
char inv_upgrade_file_type = 0;
rst_code zs_write_start_frame(int fd, unsigned char *data_ptr, unsigned short w_len)
{
    unsigned int sum = 0;
...
rst_code zs_write_data_frame(int fd, unsigned char *data_ptr, unsigned short w_len)
{
    unsigned int sum = 0;
    unsigned short len = 0;
    unsigned int i = 0;
...
rst_code zs_write_end_frame(int fd, unsigned char *data_ptr, unsigned short w_len)
{
    unsigned int sum = 0;
    unsigned short len = 0;
    unsigned int i = 0;
...
rst_code comm_write_data(int fd, Inv_cmd cmd, const void *data_ptr, unsigned short len)
{
...
void update_info_parse(update_info *update_info)
{
    char *head_ptr = NULL;
...
void inv_update(char *inv_fw_path)
{
    ...
    if (inv_real_num == 1 && g_p2p_mode > 0)
    {
