...
#define INV_NUM 15
#define FULL_HOST_PDK "hdfdZ296Pfl"
...
#define ALIYUN_FDBG_MASK 0x0001
#define CMD_INV_ONOFF 0x0002
...
typedef struct tagInv_data
{
    ...
    uint16_t pac_out_percent;   ///< the check time of inverter
...
typedef struct
{
    ...
    char host[128]; //tianhe_host
    int port;       //tianhe_port
    //int req_time_ms;
    //int keep_alive_ms;
    //int write_buf_size;
    //int read_buf_size;
...
/** tianhe tcp reference*/
#include "stdint.h"
typedef struct
{
    uint16_t cmd;
    void (*hook)(uint8_t *, int);
} recv_cmd_tab_t;
