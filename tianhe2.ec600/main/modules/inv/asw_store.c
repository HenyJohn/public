...
#define TIANHE_REG_KEY "tianhe_reg"
#define TIANHE_ONOFF_KEY "tianhe_onoff"
...
asw_db_open(void)
{
    ...
    if (xSemaphoreTake(xSemaphore, (TickType_t)10000 / portTICK_RATE_MS) == pdTRUE)
    {
...
{
    if (xSemaphoreTake(xSemaphore, 10000 / portTICK_RATE_MS) == pdTRUE) //10 / portTICK_RATE_MS,(TickType_t)100
    {
        esp_err_t err;
        int error_code = 0;
        char key[16] = {0};
        ...
        case NVS_3RD_CA_CERT:
            ...
            break;
        case NVS_TIANHE_REG:
            memcpy(key, TIANHE_REG_KEY, strlen(TIANHE_REG_KEY));
            blob_len = sizeof(uint32_t);
            my_handle = ate_nvs_handle;
            break;
        case NVS_TIANHE_ONOFF:
            memcpy(key, TIANHE_ONOFF_KEY, strlen(TIANHE_ONOFF_KEY));
            blob_len = sizeof(char);
            my_handle = ate_nvs_handle;
            break;
        default:
            goto RES_ERR;
...
{
    if (xSemaphoreTake(xSemaphore, 10000 / portTICK_RATE_MS) == pdTRUE)
    {
        esp_err_t err;
        int error_code = 0;
        char key[16] = {0};
...
int get_device_sn(char *sn)
{
    ...
    {
        memcpy(sn, para.psn, 24);
        return 0;
    }
    return -1;
}

uint32_t get_tianhe_reg_state(void)
{
    uint32_t tianhe_reg_ok = 0;
    general_query(NVS_TIANHE_REG, &tianhe_reg_ok);
    return tianhe_reg_ok;
}

void set_tianhe_reg_state(uint32_t *var)
{
    general_add(NVS_TIANHE_REG, var);
}
...
int get_tianhe_onoff(void)
{
    char var = -1;
    int res = general_query(NVS_TIANHE_ONOFF, &var);
    if (res == 0 && (var == 0 || var == 1))
    {
        return var;
    }
    return 1;
...
void set_tianhe_onoff(char var)
{
    if (var == 0 || var == 1)
    {
        char x = var;
        general_add(NVS_TIANHE_ONOFF, &x);
    }
}
