...
int recive_invdata(int *flag, Inv_data *inv_data, int *lost_flag)
{
    ...
    if (mq0 != NULL)
        if (xQueueReceive(mq0, &invdata, (TickType_t)10) == pdPASS)
        {
            ...
            /** 读取队列的数据*/
            printf("mqtt status: %d\n", cat1_get_tcp_status() == 1);
            printf("netework_state: %d\n", !netework_state);
            printf("monitor_state: %d\n", monitor_state && INV_SYNC_PMU_STATE);
            if ((cat1_get_tcp_status() == 1) && (!netework_state || (monitor_state && INV_SYNC_PMU_STATE))) //net ok
            {
...
memcpy(buff, (char *)inv_data, sizeof(Inv_data));
    ...
    /** 队列无数据*/
    if ((cat1_get_tcp_status() == 1) && (!netework_state || (monitor_state && INV_SYNC_PMU_STATE)) && get_second_sys_time() > 300) //query data
    {
        *flag = 0;
...
int _3rd_pmu_info_publish(int index, char *pub_topic)
{
    ...
    res = cat1_mqtt_pub_qos_0(index, g_cst_pub_topic, payload);
    if (res == 0)
        return 0;
    else
    {
