...
void getdevdata_handler(httpd_req_t *req, http_req_para_t *para)
{
    ...
    else if (strcmp(para->device, "3") == 0)
    {
        ...
        cJSON_AddNumberToObject(res, "iet", inv_meter.invdata.h_total);
        cJSON_AddNumberToObject(res, "oet", inv_meter.invdata.e_total);
...
void getdev_handler(httpd_req_t *req, http_req_para_t *para)
{
    if (strcmp(para->device, "1") == 0 || strlen(para->device) == 0)
    {
        if (strcmp(para->secret, "1") == 0 || strlen(para->secret) == 0)
        {
            ...
            cJSON_AddStringToObject(res, "host", setting.host);
            cJSON_AddNumberToObject(res, "port", setting.port);
            ...
            cJSON_AddNumberToObject(res, "status", cat1_get_tcp_status() == 1 ? 0 : -1);
            cJSON_AddNumberToObject(res, "csq", cat1_get_csq());
            cJSON_AddStringToObject(res, "iccid", g_ccid);
            cJSON_AddNumberToObject(res, "ate_net", g_ate_net_status);
...
"value": 
        ...
        "ali_port": 1883,
        "pdk": "qFJmHiMNjjr",
        "ser": "06779163914200e3a38be000a96b0749",
        "tianhe_ip": "psinf.trinasolar.com",
        "tianhe_port": 3002
*/
            ...
            wifi_ap_para_t ap_para = {0};
            sprintf(ap_para.ssid, "%s%s", "Trinasolar-", setting.psn + strlen(setting.psn) - 4);
    ...
    else if (device == 3)
    {
        if (strcmp(action, "setmeter") == 0)
        {
            ...
            // getJsonNum(&meter.date, "regulate", value);
            // printf("regulate: %d\n", meter.reg);
...
int ate_check_net(void)
{
    struct tm tim = {0};
    char *url = "http://mqtt.aisweicloud.com/api/v1/updatetime?psno=B300019A0095&sign=MLOOONFNOOFJOOFJ";
    return cat1_http_get_time(url, &tim);
}
...
void fdbg_handler(httpd_req_t *req, http_req_para_t *para)
{
    ...
    if (json != NULL)
    {
        ...
        // print_bytes_as_hex("parse bytes", fdbg_msg.data, fdbg_msg.len);
        /** 2022.7.8 forbid user on if remote off **/
        if (get_tianhe_onoff() == 0)
        {
            if (is_found_onoff_cmd(ascii_data, strlen(ascii_data)))
            {
                goto EEE;
            }
        }
                ...
                if (to_cgi_fdbg_mq != NULL)
                {
                                      ...
                                      10000 / portTICK_RATE_MS) == pdPASS)
                    {
                        if (fdbg_msg.type == MSG_FDBG)
                        {
                            ...
                            /** 2022 test on off */
                            if (is_found_onoff_cmd(ascii_data, ascii_len))
                            {
