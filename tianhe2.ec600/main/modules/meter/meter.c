...
int md_decode_meter_pack(uint_8 metmod, uint_8 salveaddr, uint_8 funcode, uint_8 len, uint_8 *buffer, char type)
{
    ...
    //meter_data->e_total_out = (sint_32)(f_tmp*10);
    inv_meter.invdata.h_total = (sint_32)(f_tmp * 10); //meter E-Total( out) (old msg)
    printf("e_total------------------%d\n", inv_meter.invdata.e_total);
    printf("h_total------------------%d\n", inv_meter.invdata.h_total);
    ...
    printf("con_stu------------------%d\n", inv_meter.invdata.con_stu);
    printf("e_today------------------%d\n", inv_meter.invdata.e_today);
...
char md_query_meter_data(unsigned char meter_mode, unsigned char slaveadd, char type)
{
    ...
    int res = recv_bytes_frame_waitting_fast(UART_NUM_1, buffer, &recv_len);
...
int query_meter_proc(void)
{
    ...
    // if(0==parse_sync_time())
    //         if(get_current_days() != mtconfig.date)
    //             mtconfig.reg=0;
    // //general_query(NVS_METER_SET, (void *)&mtconfig);
    //printf("read ---->enb:%d, mod:%d, regulate:%d, target:%d, date:%d \n", mtconfig.enb, mtconfig.mod, mtconfig.reg, mtconfig.target, mtconfig.date);
    //writeReative_factory(inv_meter.invdata.cosphi)
    if (mtconfig.enb == 1)
    {
        if (10 == mtconfig.reg)
        {
            printf("(10 == mtconfig.reg) %d \n", mtconfig.reg);
            if (read_time >= 10)
            {
                ret = md_query_meter_data(mtconfig.mod, 1, 0);
                ...
                ret = md_query_meter_data(mtconfig.mod, 1, 1);

            if ((0 == ret) && (10 == mtconfig.reg))
            {
                energy_meter_control(mtconfig.target);
            }
...
int energy_meter_control(int set_power)
{
...
static rst_code md_write_SIG_command(const Inverter *inv_ptr, const void *data_ptr, uint16_t len)
{
    ...
    data_len = 8;
    memset(buffer, 0, 256);
    int res = recv_bytes_frame_waitting(UART_NUM_1, buffer, &data_len);
