...
int get_meter_mem_data(Inv_data *data)
{
    ...
    if (inv_meter.invdata.status == 0)
    {
        memcpy(data, &inv_meter.invdata, sizeof(Inv_data));
        data->pac = -(int)(data->pac);
    }
    else
    {
...
int query_meter_proc(void)
{
    ...
    if (!mtconfig.date)
    {
        ...
        if (0 == parse_sync_time() && sync_meter_config == 0)
        {
            ...
            if (mtconfig.enb == 1 && strlen(inv_meter.invdata.psn) == 0)
                get_meter_modelname(mtconfig.mod, inv_meter.invdata.psn);
        }
    }
